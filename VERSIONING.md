...
The Python package reads the version from `ambilight/_version.py` (auto-generated by build.py):
...
The `_version.py` file is auto-generated by build.py from the VERSION file:

```python
# ambilight/_version.py (auto-generated)
__version__ = "1.0.2"
```

**Why this approach?**
- The `_version.py` file is **bundled into PyInstaller** artifacts, so it works in frozen builds
- During development, build.py automatically syncs the VERSION file to `_version.py`
- No runtime file access needed (unlike reading VERSION at runtime)
...
version_py = ROOT / "ambilight" / "_version.py"  # Sync Python version
version_py.write_text(f'__version__ = "{canonical_version}"')  # Auto-generate
```
...
- **Role**: Canonical version source (single source of truth)
- **Synced by**: build.py during every build

### `ambilight/_version.py` File

- **Location**: ambilight package root
- **Format**: Auto-generated Python file with `__version__ = "X.Y.Z"`
- **Purpose**: Bundled in PyInstaller artifacts (enables frozen builds to access version)
- **Generated by**: build.py _check_version_sync() during every build
- **Imported by**: ambilight/__init__.py
- **Never edit manually**: This file is regenerated on every build
